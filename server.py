...
class Validator:
    def check_pw(self, password = "", confirm_pw = ""): # password and confirm_pw values from request.form
        flag = False
        while True:
            if (len(password)<8):
                flag = True
    ...
    def check_reg(self, email): # email values from request.form
        ...
        query = mysql.query_db("SELECT * FROM users WHERE email like '%(email)s';")
        flag = False
        if query:
    ...
    def check_name(self, first_name, last_name): # first & last name from request.form
        flag = False
...
class QuerySearch:
    def user_add(self, form): # add user to db
        ...
        try:
            ...
            mysql = connectToMySQL("travel_bug")
            user = mysql.query_db("SELECT id, first_name, last_name, email, avatar FROM users;")
            return user
        except:
            return False
    ...
    def user_pins(self, user_id): # get users 10 most recent pins
        mysql = connectToMySQL("travel_bug")
        query = mysql.query_db("SELECT pins.post, pins.created_date, pins.updated_date, pins.go, pins.avoid, pins.picture, users.first_name, users.last_name, locations.location FROM pins LEFT JOIN users ON pins.user_id = users.id LEFT JOIN locations ON pins.location_id = locations.id WHERE pins.user_id like %(user_id)s SORT BY pins.created_date DESC LIMIT 10;")
        return query
    
    def user_get(self, user_id):
        mysql = connectToMySQL("travel_bug")
        query = mysql.query_db("SELECT * FROM users WHERE users.id like %(user_id)s;")
        return query
    ...
    def pin_delete(self, user_id, pin_id):
        ...
        if query["user_id"] == user_id:
            ...
            mysql = connectToMySQL("travel_bug")
            query = mysql.query_db("DELETE FROM pins WHERE pins.id LIKE %(pin_id)s;")
            mysql = connectToMySQL("travel_bug")
    ...
    def pin_update(self, form):
        ...
        query = mysql.query_db("SET SQL_SAFE_UPDATES = 0;")
        mysql = connectToMySQL("travel_bug")
        query = "UPDATE pins SET location_id = '%(location_id)s', post = '%(post)s', go = '%(go)s', avoid = '%(avoid)s', updated_date = NOW(), picture = '%(picture)s' WHERE pins.id LIKE %(pin_id)s;"
        data = {
